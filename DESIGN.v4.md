...
claude --bare -p "" \
  ...
  --mcp-config <path> \
  --agents <json> \
  --allowedTools "Read,Edit,Bash,mcp__moai__*" \
...
이로써 moai-cli 가 **결정론적 실행 환경**을 보장한다.
...
### 피벗 2: **IDE MCP Server Pattern 이 PRIMARY 통합 경로**

VS Code 확장과 JetBrains 플러그인이 이미 하는 방식. 공식 문서 ([vs-code](https://code.claude.com/docs/en/vs-code)):
...
**이것이 Anthropic 이 공식적으로 지원하는 유일한 IDE 통합 아키텍처다.** v3 에서 "fallback" 이라 부른 것을 v4 에서 **PRIMARY** 로 승격한다.

```
┌──────────────────────────────────────────────────────┐
│         moai-cli 호스트 프로세스 (macOS)              │
│                                                      │
│  ┌──────────────┐    ┌────────────────────────────┐  │
...
moai-cli 는 VS Code 확장과 **정확히 같은 패턴**으로 Claude Code 와 대화한다. wire format (MCP + JSON) 은 공개 표준이므로 안정적이다.
...
7. **CLI reuse 가능성**: Rust core 는 나중에 CLI 버전 (`moai-cli-headless`) 으로 재사용 가능. Swift UI 없이 Rust 만으로 동작.
...
| IDE Lockfile = "fallback" | **PRIMARY** 경로로 승격 | [vs-code](https://code.claude.com/docs/en/vs-code) |
...
| 라운드 | 결정 | 확정값 |
|---|---|---|
| 1 | 포지셔닝 | moai-adk 공식 macOS GUI, 모노레포 (`modu-ai/moai-adk` 내부 `moai-cli/` 서브디렉토리) |
| 2 | 라이선스 | **MIT** |
...
**v4 약속**: M0 ~ M8, 전 마일스톤 macOS 만. `moai-cli-linux`, `moai-cli-wsl`, `moai-cli-windows` 는 **로드맵에 존재하지 않음**.
...
> **moai-cli**: moai-adk 의 공식 macOS 네이티브 IDE-쉘. Claude Code 를 subprocess 로 호스트하여 27개 hook 이벤트 + 26 전문 에이전트 + TRUST 5 품질 게이트 + @MX 태그 시스템 + Kanban/SPEC 워크플로우를 한 화면에서 시각화·조작한다.
...
- ✅ OK: "moai-cli", "MoAI Agent IDE", "moai + Claude", "Powered by Claude"
...
> cmux 가 터미널 멀티화 + 브라우저 surface 를 잘 한다면, moai-cli 는 그 위에 **SPEC/TRUST/@MX/Kanban 워크플로우 시각화** 와 **Claude 가 직접 UI 를 조작할 수 있는 공식 IDE MCP Server Pattern** 을 올린다.
...
5. **moai-adk 는 건드리지 않고 얹는다**. moai-cli 는 `moai` Go 바이너리를 worktree 연산을 위해 subprocess 로 호출할 뿐. `claude` 를 SDK 로 호스팅.
...
│   ├── moai-plugin-installer/     # moai-cli-plugin 자동 설치
...
pub async fn spawn_claude(ws: &Workspace) -> Result<ClaudeHost> {
    ...
    cmd.args(&[
        ...
        "--allowedTools", "Read,Edit,Write,Bash,Glob,Grep,mcp__moai__*,mcp__ide__*",
    ]);
...
```json
// moai-cli-plugin/.claude-plugin/plugin.json
{
  "name": "moai-cli",
  "version": "0.1.0",
  ...
  "userConfig": {
    "ide_port": { "type": "number", "title": "moai-cli IDE server port" },
...
### 4.5 IDE MCP Tool 카탈로그 (moai-cli 가 Claude 에 노출)
...
Claude Code 가 LSP 서버를 spawn + 관리. moai-cli Code Viewer 는 `mcp__ide__getDiagnostics` tool 호출로 진단을 받아옴. **자체 LSP 클라이언트 구현 불필요** — 이것이 v4 의 큰 단순화 중 하나.
...
- **소유권**: PTY master = Ghostty. moai-cli 는 attach view.
- **GPU**: Metal 60fps@4K
...
- Secondary: `.moai/logs/task-metrics.jsonl` tail (moai-cli 꺼진 동안 복구)
...
├── mcp-config.json                  # moai-cli IDE server 연결
├── lsp.json                         # 6개 언어 LSP
├── commands/
│   ├── kanban.md                    # /moai-cli:kanban
│   ├── memory.md                    # /moai-cli:memory
│   ├── connect.md                   # /moai-cli:connect
│   └── surface.md                   # /moai-cli:surface
├── skills/
│   ├── moai-cli-open-workspace/SKILL.md
│   └── moai-cli-focus-agent/SKILL.md
├── output-styles/
...
│   │   ├── install-plugin.sh        # ~/.claude/plugins/moai-cli@local/
│   │   └── reload.sh
│   │
│   ├── tests/
...
└── .github/workflows/
    ├── ci-go.yml                    # 기존 Go CI
    ├── ci-moai-cli-rust.yml         # cargo test
    ├── ci-moai-cli-swift.yml        # Xcode build + test
    └── release-moai-cli.yml         # notarize + DMG + Sparkle
```

**Path filter**: `moai-cli/**` 변경 시만 CLI CI 트리거. Go CI 와 독립.
...
| **M4 Claude 통합 심화** | 3주 | moai-cli-plugin 자동 설치, Native permission dialog, Bash input rewriter, LSP 6개 언어 등록, cost tracker, output style forceForPlugin |
...
**주 2:**
- D6: `claude --mcp-config moai-cli.json -p "..."` 실행 → moai-cli MCP server 에 auto-connect 확인
...
- `--settings`, `--mcp-config`, `--agents`, `--allowedTools`, `--permission-mode` 플래그 동작 확인
...
| 축 | cmux | Warp | Wave | Zed | Ghostty | **moai-cli v4** |
|---|---|---|---|---|---|---|
...
| Claude 통합 | teammate shim | cloud agents | badge rollup | ACP (hook 미지원) | 없음 | **공식 IDE MCP Server Pattern** |
...
### 13.1 moai-cli 의 7가지 moat

1. **공식 IDE MCP Server Pattern 채택** — Anthropic 공식 문서화 패턴, wire format 안정
...
### O1. MCP 서버 Rust 라이브러리 선택

- **옵션 A**: `rmcp` crate ([UNVERIFIED] 존재 확인 필요)
- **옵션 B**: `axum` + `jsonrpsee` 로 MCP 레이어 자체 구현
- **옵션 C**: `mcp-rust-sdk` 커뮤니티 라이브러리

**Pre-M0 Day 2 에서 결정.** `rmcp` 가 존재하면 A, 없으면 B.

### O2. swift-bridge vs uniffi-rs

- **옵션 A**: `swift-bridge` (Rust → Swift 특화, compile-time 생성)
- **옵션 B**: `uniffi-rs` (Mozilla, Kotlin/Swift/Python 공통)
- **옵션 C**: `cbindgen` + 수동 wrapper

**권장**: A. Swift-only 이므로 swift-bridge 의 single-target 최적화가 이점.
...
- 처음 moai-cli 실행 시 `~/.claude/plugins/moai-cli@local/` 자동 drop
- settings.json 의 `enabledPlugins` 에 `moai-cli@local: true` 추가
- **Onboarding 화면에서 명시적 체크박스** 로 사용자 동의
- 동의 없이 스킵 시: moai-cli 는 IDE MCP server 만 작동 (hook 브리징 비활성화)

### O5. `claude` 바이너리 버전 pinning 정책

- **권장**: 최소 버전만 지정 (`claude >= 2.2.0`)
- Breaking change 감지 시 `claude >= 2.2.0 < 2.3.0` 형태로 상한 추가
- moai-cli 가 `claude --version` 을 M0 초기에 확인, 미설치 시 설치 가이드
...
**M4 완료 전까지 결정.** 제약: "Claude Code" 명칭 금지.
...
- `modu-ai/moai-adk` README 에 moai-cli v4 로드맵 섹션 추가
- M4 완료 시점에 HN 예고
...
### 제품
**moai-cli**: moai-adk 의 공식 macOS 네이티브 IDE-쉘. Claude Code 를 subprocess 로 호스트. MIT 라이선스. **macOS 영구 단독**.

### 3가지 핵심 피벗 (v3 → v4)
1. **"SDK 임베드" → "subprocess 호스트"** (공식 Agent SDK 조차 subprocess 사용)
2. **IDE MCP Server Pattern** 을 PRIMARY (VS Code 와 같은 공식 패턴)
...
- **IDE MCP Server**: 127.0.0.1:<random> + `~/.claude/ide/<port>.lock` + bearer token
