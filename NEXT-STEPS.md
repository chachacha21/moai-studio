# moai-cli — Next Steps (4 단계)

> **목적**: DESIGN.v4.md 승인 후 진행할 4개 작업을 구체적으로 명시.
> **Date**: 2026-04-11
> **Status**: Ready to execute

이 문서는 형님이 `moai init` 으로 moai-cli 저장소를 신규 프로젝트로 초기화한 후 **순차 실행할 4 단계 작업**을 정의합니다.
...
# Xcode 프로젝트
# (수동) open Xcode → New macOS App → moai-cli.app
```
...
| # | 결정 | 의존 | 기한 |
|---|---|---|---|
| O1 | MCP 서버 Rust 라이브러리 (`rmcp` vs `axum+jsonrpsee`) | 단계 1.2 결과 | M0 D5 |
| O2 | `swift-bridge` vs `uniffi-rs` vs `cbindgen` | 없음 (M0 D8 전) | M0 D1 |
...
| O6 | 브랜딩 최종 확정 (moai-cli vs moai-ide vs moai-shell vs MoAI Studio) | 없음 | M4 완료 전 |
...
**목표**: moai-cli 존재를 알리고 early adopter 를 유치. **단, M4 (Claude 통합 심화) 완료 전까지는 공개 마케팅 자제.**
...
- **Target**: macOS 14+
- **License**: MIT
- **Repo**: [moai-cli](https://github.com/modu-ai/moai-cli)
```

### 작업 4.2 — GitHub 저장소 공개

- `~/moai/moai-cli` → `modu-ai/moai-cli` push
- README.md, DESIGN.v4.md, NEXT-STEPS.md, research/ 모두 공개
- Issues, Discussions 활성화

### 작업 4.3 — Show HN 예고 포스팅 (M5 완료 시점)

제목: "moai-cli: A native macOS IDE shell for Claude Agent workflows"
...
공식 문서의 브랜딩 가이드라인 ("Claude Code" 명칭 금지) 준수 여부 확인 겸 introduction. moai-cli 가 공식 IDE MCP Server Pattern 을 모범적으로 구현한 사례로 제시 가능.

### Deliverables

- `modu-ai/moai-adk` README 업데이트 PR
- `modu-ai/moai-cli` GitHub 저장소 공개
- Show HN draft 포스트
- cmux outreach 기록
...
- [ ] O2 (swift-bridge vs uniffi-rs) — M0 D1 전에 결정 필요 (권장: swift-bridge)
- [ ] O6 (브랜딩) — 지금은 "moai-cli" 로 유지, M4 전에 최종 확정
